...
import irden.space.proxy.plugin.irden.integration.web.client.linker.IrdenAppLinkerClient;
...
public class SiteLinker {
    private final IrdenAppLinkerClient irdenAppLinkerClient;
    ...
    public boolean link(Player player, String secret) {
        LinkPlayerRequest build = LinkPlayerRequest.builder()
                ...
                .secret(secret)
                .build();
        LinkPlayerResponse linkResult = irdenAppLinkerClient.link(build);
    ...
    public boolean unlink(Player player) {
        irdenAppLinkerClient.unlink(new PlayerUuidParam(player.uuid().toString()));

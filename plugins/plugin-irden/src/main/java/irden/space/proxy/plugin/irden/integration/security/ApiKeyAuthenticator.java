package irden.space.proxy.plugin.irden.integration.security;
...
public class ApiKeyAuthenticator implements RestAuthenticator {
    ...
    private final IrdenConfig config;
    ...
    public void register() {
        if (config.siteConnector().inboundApiKey() == null || config.siteConnector().inboundApiKey().isBlank()) {
    ...
    public AuthenticationResult authenticate(AuthenticationRequest request) {
        String expected = config.siteConnector().inboundApiKey();

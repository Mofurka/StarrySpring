...
public enum EntityMessageTargetCodec implements BinaryCodec<EntityMessageTarget> {
    ...
    public EntityMessageTarget read(BinaryReader reader) {
        ...
        if (targetUnique) {
            return new UniqueEntityIdTarget(StarStringCodec.INSTANCE.read(reader));
        }
    ...
    public void write(BinaryWriter writer, EntityMessageTarget target) {
        switch (target) {
            case UniqueEntityIdTarget(String uniqueEntityId) -> {
                writer.writeBoolean(true);
                StarStringCodec.INSTANCE.write(writer, uniqueEntityId);
            }
